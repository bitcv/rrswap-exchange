...
export default function AddLiquidity({
  ...
  async function onAdd() {
    ...
    console.log(args)
    /*const estimateRet = await estimate(...args, value ? { value } : {})
    const response = await method(...args, { ...(value ? { value } : {}), gasLimit: calculateGasMargin(estimateRet) })

    function processResponse(response: TransactionResponse) {
    ...
    return noLiquidity ? (
      ...
      <AutoColumn gap="20px">
        ...
        <TYPE.italic fontSize={12} textAlign="left" padding={'8px 0 0 0 '}>
          {t('estimateTip')} {allowedSlippage / 100}% {t('revertTip')}
        </TYPE.italic>
      </AutoColumn>
    )
  }
          ...
          <TransactionConfirmationModal
            ...
            content={() => (
              <ConfirmationModalContent
                title={noLiquidity ? t('createPoolTitle') : t('receiveTitle')}
                onDismiss={handleDismissConfirmation}
                ...
                <GreyCard padding="0px" borderRadius={'20px'}>
                  <RowBetween padding="1rem">
                    <TYPE.subHeader fontWeight={500} fontSize={14}>
                      {noLiquidity ? t('initPrice') : t('price')} {t('and')} {t('poolShare')}
                    </TYPE.subHeader>
                  </RowBetween>{' '}
